  ...
  describe 'GET /show' do
    context 'when the request is valid' do
      ...
      it 'returns the exam with a 200 status' do
        get "/api/v1/exams/1?first_name=#{first_name}&last_name=#{last_name}&phone_number=#{phone_number}&college_id=#{college_id}&start_time=#{start_time}"
    ...
    context 'When the first_name param is missing' do
      ...
      it 'raises a bad request exception with a 400 status code' do
        expect { 
          get "/api/v1/exams/1?last_name=#{last_name}&phone_number=#{phone_number}&college_id=#{college_id}&start_time=#{start_time}"
        }.to raise_error ActionController::BadRequest
      end
    end
    ...
    context 'When the last_name param is missing' do
      ...
      it 'raises a bad request exception with a 400 status code' do
        expect { 
          get "/api/v1/exams/1?first_name=#{first_name}&phone_number=#{phone_number}&college_id=#{college_id}&start_time=#{start_time}"
        }.to raise_error ActionController::BadRequest
      end
    end
